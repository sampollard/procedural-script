...
\pagestyle{fancy}
\lhead{Sam Pollard}
\rhead{CS 580 -- Fall 2015 -- Matthews \\ \today}
\cfoot{\thepage}
...
\begin{tikzpicture}[scale=2]%,cap=round,>=latex]
	...
	\coordinate (u) at (-.4,1);
	\coordinate (uv) at (-1,1.2);
    % Draw a border around the curve
    \draw[double distance=8mm,line width=1pt,line join=round,line cap=round,blue] (p0) .. controls (p1) and (p2) .. (p3);
	% Draw the bezier curve
	\draw[line width=1pt] (p0) .. controls (p1) and (p2) .. (p3);
	% Mark and label the control points
    \filldraw[red] (p0) circle[radius=1.5pt] node[black, anchor=west, xshift=2mm, yshift=-5mm] {$p_0$};
    ...
    \filldraw[red] (p3) circle[radius=1.5pt] node[black, anchor=west, xshift=2mm, yshift=-5mm] {$p_3$};
    % Draw the point that we will check
    \filldraw[green] (u) circle[radius=1.5pt];
    \draw [-latex, black] (uv) node[anchor=east, xshift=1mm] {$q$} -- (u);
\end{tikzpicture}
\end{center}
